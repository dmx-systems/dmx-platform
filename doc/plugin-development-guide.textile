...
* "Overview and set up":#overview-and-set-up
...
** "Reference: plugin.properties":#reference-plugin-properties


h1(#overview-and-set-up). Overview and set up


h2(#what-a-plugin-can-do). What a plugin can do
...
h2(#setting-up-the-development-environment). Setting up the development environment
...
h2(#starting-a-plugin-project). Starting a plugin project
...
h2(#writing-the-plugin-pom). Writing the plugin POM
...
bc.. <project>
  ...
  <version>0.4</version>                          # artifact ID, and version number.
  ...
  <parent>                                        # Relates to the parent POM.
    ...
    <artifactId>deepamehta3-parent</artifactId>   # Just update the version number, if a
    <version>0.4</version>                        # newer version of DeepaMehta is available.
  </parent>

  <dependencies>                                  # Most DeepaMehta 3 plugin projects
    <dependency>                                  # depend on the DeepaMehta 3 Core module.
      <groupId>de.deepamehta</groupId>            # Copy this declaration as is.
      <artifactId>deepamehta3-core</artifactId>   # Just update the version number, if a
      <version>0.4</version>                      # newer version of DeepaMehta is available.
...
h2(#building-the-plugin). Building the plugin
...
bc. deepamehta3-topicmaps/                        # Plugin project directory.
  ...
  target/                                     # Contains artifacts build by Maven.
    deepamehta3-topicmaps-0.4.jar             # The jar ready for deployment in OSGi console.


h2(#deploying-the-plugin). Deploying the plugin
...
bc. g! start file:/home/terry/deepamehta3-dev/deepamehta3/deepamehta3-topicmaps/target/deepamehta3-topicmaps-0.4.jar
...
h2(#distributing-your-plugin). Distributing your plugin

Upload your plugin, namely the jar file, to a webserver and advertise the jar file's URL.


h2(#installing-3rd-party-plugins). Installing 3rd party plugins
...
h1(#developing-a-plugin). Developing a plugin


h2(#reference-plugin-properties). Reference: plugin.properties

|_. Property      |_. Meaning     |_. Mandatory / Optional |
^.| @pluginPackage@
  | Name of the plugin's Java main package, e.g. @com.example.deepamehta3.topicmaps@.
    This is the package that contains possible @migrations@ and @resources@ packages.
    <br>
    This property is only required if
    1) The plugin provides _no_ Plugin subclass, and
    2) The plugin _provides_ migration classes or REST resource classes.
  | Optional. |
^.| @requiredPluginMigrationNr@
  | The number of the migration the plugin requires to run, e.g. @1@.
    <br>
    This property is only required if the plugin provides migration classes or declarative migrations.
  | Optional. |
^.| @restResourcesNamespace@
  | Namespace to map the plugin's REST resources to, e.g. @/topicmap@.
    <br>
    This property is only required if the plugin provides REST resource classes.
  | Optional. |
^.| @clientSidePluginFile@
  | The filename of the plugin's client-side part (a JavaScript file), e.g. @topicmaps_plugin.js@
    <br>
    This property is only required if the plugin provides a client-side part.
  | Optional. |
