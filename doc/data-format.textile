...
<br><br>
*Note:* when a topic type is requested via the DeepaMehta REST API 2 additional attributes are returned in the response topic type: @id@, and @type_uri@.
...
2. A topic and a relation have an additional @ref_id@ attribute which is required when updating the topic resp. relation references (e.g. when moving a topic on the topic map or removing a relation from the topic map).
