...
^.| @editable@
  | Is the data field's value editable by the user?
  | @true@ (default)
    @false@
  | Optional. |
^.| @editor@
...
^.| @KEY@
  | The exact property value is indexed under the property's key (the data field URI).
    <br>
    Use this mode if you want retrieve an unique topic by property value.
    <br>
    Topics indexed this way can be retrieved by the "getTopic" service call:
    @Topic getTopic(String key, Object value)@.
    <br>
...
^.| @FULLTEXT@
  | The property value is tokenized and each token is indexed under no particular key.
    ...
    <br>
    Topics indexed this way can be retrieved by the "searchTopics" service call:
