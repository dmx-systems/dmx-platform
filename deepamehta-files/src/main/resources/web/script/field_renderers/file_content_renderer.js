    ...
    function render_content(field_value_div) {
        try {
            ...
            if (media_type == "text/plain") {
                ...
                return $("<img>").attr("src", local_resource_URI())
            } else if (media_type == "application/pdf") {
                return $("<embed>").attr({src: local_resource_URI(), width: "100%", height: dm4c.canvas.canvas_height})
                // return $("<iframe>").attr({src: local_resource_URI(), width: "100%",
                //     height: dm4c.canvas.canvas_height, frameborder: 0})
            } else if (js.begins_with(media_type, "audio/")) {
                return $("<embed>").attr({src: local_resource_URI(), width: "95%", height: 32})
                // var content = "<audio controls=\"\" src=\"" + local_resource_URI() + "\"></audio>"
            } else if (js.begins_with(media_type, "video/")) {
                return $("<embed>").attr("src", local_resource_URI())
