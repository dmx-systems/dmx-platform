function dm3_filemanager() {
    ...
    this.process_files_drop = function(files) {
        ...
        for (var i = 0; i < dir_count; i++) {
            dm3c.get_plugin("files_plugin").create_file_topics(files.get_directory(i), i == 0)
        }
        for (var i = 0; i < files.get_file_count(); i++) {
            dm3c.get_plugin("files_plugin").create_file_topic(files.get_file(i), !dir_count && i == 0)
        }
        //
