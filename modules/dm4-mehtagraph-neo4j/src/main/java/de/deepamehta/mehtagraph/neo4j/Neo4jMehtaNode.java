...
class Neo4jMehtaNode extends Neo4jMehtaObject implements MehtaNode {
    ...
    public void indexAttribute(MehtaGraphIndexMode indexMode, String indexKey, Object value, Object oldValue) {
        if (indexMode == MehtaGraphIndexMode.OFF) {
            ...
            if (oldValue != null) {
                topicContentExact.remove(node, indexKey, oldValue);            // remove old
            }
            topicContentExact.add(node, indexKey, value);                      // index new
            ...
            if (oldValue != null) {
                topicContentFulltext.remove(node, KEY_FULLTEXT, oldValue);     // remove old
            }
            topicContentFulltext.add(node, KEY_FULLTEXT, value);               // index new
        } else if (indexMode == MehtaGraphIndexMode.FULLTEXT_KEY) {
            if (oldValue != null) {
                topicContentFulltext.remove(node, indexKey, oldValue);         // remove old
            }
            topicContentFulltext.add(node, indexKey, value);                   // index new
        } else {
